...
公共素材层须标记 `origin=public_stock` 并指向已校验的 `asset_record`。若请求结果为 `not_found` 或 `rejected`，该层不得出现在清单中；回到 Director 阶段补充主视觉，Remotion 不得随意生成替代物。

## 单张素材的派生动画

原图完整使用优先。只有在拆开后仍自然、边缘清晰，并且确实增强语义或运动关系时，才允许从单张素材派生局部裁切、视差或动画组件；不得为了凑三层而抠坏物件、硬切完整画面或制造廉价近景。

同一张图及其所有裁切、遮罩、抠图和派生组件属于同一素材组，只算一个独立来源，不能增加独立层数。若拆层不满足质量条件，保留原图完整呈现，并使用其他独立素材或 Director 视觉补足镜头关系。
