...
每个解释型拼贴镜头至少需要 3 个独立视觉层，并覆盖背景、结构/关系、主物件/信息、前景/强调中的至少 3 类。至少一个层必须是具备真实质感的图片、照片、抠图、纸媒或视频资产；SVG/CSS 只能辅助，不能用三个抽象形状凑够数量。

公共素材层须标记 `origin=public_stock`，并在项目中保留可校验的素材记录：来源页、下载 URL、许可/使用说明、下载时间、文件指纹、语义用途与图层职责。检索无匹配时必须记录 `not_found` 并取消该层，不能用近似素材或抽象图形替代。

## 单张素材的派生动画

原图完整使用优先。只有画面天然存在可独立物件、背景/前景或明确分区时，才允许从同一原图做局部抠取、局部移动或遮罩动画；不得为了凑层数硬抠、硬拆或把同一图的裁切版本称为多个独立来源。同一张图及其所有裁切、抠取与变体始终只算一个独立来源。
...
已批准大纲以真人表演为主要信息的镜头可用 `base_mode=presenter_anchor`，填写 `anchor_reason`、`outline_evidence`、`source_video` 和 `remotion_component`；此类镜头不执行三层数量规则，不能将解释型拼贴谎报为锚点。其原声、实际关键帧与用户审核仍须通过 [送审证据](review-evidence.md)。结构 PASS 从不意味着视觉、声音或送审 PASS。
