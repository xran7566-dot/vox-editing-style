...
- 参考片的逐镜、声画与节奏拆解：仅在用户明确同意本次视频上传云端分析后，可选调用 `$cine-sleuth`；详见 [CineSleuth 适配器](cine-sleuth-adapter.md)。它的报告只是参考证据，不是新片的画面模板，也不替代本地原始资料审核。
...
4. 字幕来源与确认：用户 SRT、用户文案对齐或用户确认的 `approved.srt`；未确认的转写草稿必须阻塞。
5. 粗剪候选：长静音、口误、假起句、重复重说、大气口的时间码、保留段、删除理由与置信度；细小气口、呼吸、自然/情绪停顿明确列为保留项。
6. 按原声/SRT 时间切分的语义单元：原文、关键词、核心观点、观众应理解什么、必须保留、禁止虚构。
7. 内容遗漏与错误检查：已识别关键词、已识别核心观点、可能遗漏、疑似错误/错听/时间码不一致、需要用户确认。
8. 事实/主张表：原片明说、外部已核实、待确认或只是参考假设。
9. 参考素材范围：`scene_solution`、`visual_quality`、`technical_mechanism`、`system_anchor` 或 `needs_user_decision`。必须记录本次是否允许继承上一任务的版式/风格；未明确批准时，参考图只对当前任务生效，不得自动继承到后续任务。
10. 素材缺口、不确定项和是否阻塞生产。
11. 声音方案：BGM 情绪、能量曲线、乐器/音色禁忌，以及每个语义节点允许的音效类型、落点和强度。所有声音选择必须能回指到文案语意，禁止后期凭听感自由添加。
12. 用户审批区：通过、需修改或拒绝，并记录确认时间。
...
- `caption_confirmation`：`source`（`user_srt`、`user_text_aligned` 或 `faster_whisper`）、`approved_srt_path`、`approved_by=user`、`approved_at`。
- `rough_cut_review`：`candidate_path`、`candidate_sha256`、`status=approved`、`approved_by=user`、`approved_at`，以及默认保留的 `small_breath`、`natural_pause`、`emotional_pause`。候选文件必须包含源文件指纹、时长、`semantic_review=complete`、逐项 keep/delete 决定和真实用户批准；内容或源文件变化必须重审。
...
- `source_video` 或 `source_audio` 必须存在且指纹一致。
- `srt` 或 `transcript` 至少存在一个且指纹一致。
- 没有用户确认的 SRT 时不得生成语义单元；用户提供 SRT 时也必须记录其确认来源。
- 粗剪候选必须由用户确认；不得把细小气口、自然停顿或情绪停顿写为自动删除项。
- 语义单元不重叠，不超出视频时长。
