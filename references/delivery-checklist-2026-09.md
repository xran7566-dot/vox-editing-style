...
新增八项集成测试通过：视频粗剪输出时间及字幕映射、纯音频入口、过期执行数据拒绝、未批准粗剪拒绝、内置模板接受、缺失模板拒绝、合格/被拒 B-roll 分流，以及真实 CLI 创建运行单并验证输出 props。新建 v3 自动输出执行 props 与 edited.srt；Remotion 须使用该执行数据并在预览/渲染前校验。旧 v1/v2 不自动获得新能力，保留旧文件后按需重新创建。

这里证明程序数据衔接，不宣称自定义 Remotion 项目只要传 props 就必然正确使用；画面和声音消费执行数据仍须预览验收。完整视频 Token 消耗留到最终实测。
