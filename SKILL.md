...
**每次调用的执行证据**：C 阶段产出逐镜方案，进入实际合成前按 [编导执行合约](references/directing-handoff.md) 保存 `production/shot-plan.json`，并通过 `validate_review.py --stage compose`。表中逐镜对应原话、真人选择、构图/主动作、时长和实际实现；关键帧送审再核对整段总览。新旧项目均适用，缺失就回到对应阶段补实际工作，禁止自动填 approved 或用事件表冒充镜头表。使用标准渲染/预览入口，它们调用同一检查器。
