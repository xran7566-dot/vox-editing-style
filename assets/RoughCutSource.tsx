...
export const RoughCutSource: React.FC<{src: string; timeline: CutTimeline; mediaType?: 'video'|'audio'}> = ({src, timeline, mediaType='video'}) => {
  ...
  return <>{timeline.segments.map((segment, i) => {
    ...
    return duration > 0 ? <Sequence key={i} from={from} durationInFrames={duration}>
      {mediaType === 'audio' ? <Audio src={src} startFrom={Math.round(segment.source_start * fps)}/>
